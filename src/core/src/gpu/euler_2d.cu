    ...
    float_type *p_p_next)
{
  ...
  euler_2d_calculate_next_time_step_gpu_kernel <<<grid_size, block_size>>> (
      nx, ny, dt, gamma, cell_area, edge_length,
...
#define GEN_EULER_2D_INSTANCE_FOR(type)                     \
  ...
  template void euler_2d_calculate_next_time_step_gpu (     \
      ...
      type cell_area, const type *cell_length,              \
